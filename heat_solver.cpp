...
heat_solver::heat_solver(grid& Grid, solver_inputs *solverInputs) : grid(Grid) {
	...
	u = (double **) calloc(solverInputs->n_pts, sizeof *u);

	t = (double *) calloc(nt_steps, sizeof *t);

	for(i = 0; i < solverInputs->n_pts; i++) {
		u[i] = (double *) calloc(solverInputs->nt_steps, sizeof **u);
	}
...
void heat_solver::set_initial_condition() {

	sine_initial_condition();
}

void heat_solver::solve_heat_equation() {
...
void heat_solver::sine_initial_condition() {
...
void heat_solver::finite_difference_solution() {
...
void heat_solver::identity_matrix(double **I, int n) {
...
void heat_solver::finite_difference_matrix(double **M, int n) {
...
void heat_solver::matrix_row_exchange(int n, double **A, int i, int j) {
...
void heat_solver::vector_row_exchange(int n, double *b, int i, int j) {
